...
public class LzHttpClient : NotifyBase, ILzHttpClient
{
        ...
        [CallerMemberName] string? callerMemberName = null!)
    {
        ...
        if (!methodMap.MethodMap.TryGetValue(callerMemberName!, out MethodMapEntry? methodMapEntry))
            throw new Exception($"{nameof(LzHttpClient)}.{nameof(SendAsync)} failed. callerMemberName {callerMemberName} not found in MethodMap");

        var securityLevel = methodMapEntry.SecurityLevel;
    ...
    {
        await Task.Delay(0);
        throw new NotImplementedException("AWS Signature V4 signing requires the LzHttpClientSigV4 class. Use the LazyStack.Client.Auth.Cognito.SigV4 package.");
    }
    public void Dispose()
    {
            httpClient.Dispose();
